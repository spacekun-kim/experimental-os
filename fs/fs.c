...
/*read super_block from hd to mem*/
		struct super_block* sb_buf = (struct super_block*)sys_malloc(SECTOR_SIZE);

		cur_part->sb = (struct super_block*)sys_malloc(sizeof(struct super_block));
		if(cur_part->sb == NULL) {
			PANIC("alloc memory failed in mount_partition----super_block!");
		}
		memset(sb_buf, 0, SECTOR_SIZE);
		ide_read(hd, cur_part->start_lba + 1, sb_buf, 1);

		memcpy(cur_part->sb, sb_buf, sizeof(struct super_block));
/*read block bitmap from hd to mem*/
		cur_part->block_bitmap.bits = (uint8_t*)sys_malloc(sb_buf->block_bitmap_sects * SECTOR_SIZE);
		if(cur_part->block_bitmap.bits == NULL) {
			PANIC("alloc memory failed in mount_partition----block_bitmap!");
		}
		cur_part->block_bitmap.btmp_bytes_len = sb_buf->block_bitmap_sects * SECTOR_SIZE;
		
		ide_read(hd, sb_buf->block_bitmap_lba, cur_part->block_bitmap.bits, sb_buf->block_bitmap_sects);
/*read inode bitmap from hd to mem*/
		cur_part->inode_bitmap.bits = (uint8_t*)sys_malloc(sb_buf->inode_bitmap_sects * SECTOR_SIZE);

		if(cur_part->inode_bitmap.bits == NULL) {
			PANIC("alloc memory failed in mount_partition----inode_bitmap!");
		}
...
/*1. write super block into the first sector*/
...
/*2. init bitmap then write into sb.block_bitmap_lba*/
	buf[0] |= 0x01;
...
/*3 init inode bitmap and write into sb.inode_bitmap_lba*/
	memset(buf, 0, buf_size);
	buf[0] |= 0x1;
	ide_write(hd, sb.inode_bitmap_lba, buf, sb.inode_bitmap_sects);
/*4. init inode array and write into sb.inode_table_lba*/
	memset(buf, 0, buf_size);
...
/*write root dir into sb.data_start_lba*/
	memset(buf, 0, buf_size);
...
void filesys_init(void) {
	...
	if(sb_buf == NULL) {
		PANIC("alloc memory failed in filesys_init!");
	}
...
int32_t sys_open(const char* pathname, uint8_t flags) {
	...
	if(!found && !(flags & O_CREAT)) {
		printk("in path %s, file %s isn't exist.\n", searched_record.searched_path, (strrchr(searched_record.searched_path, '/')));
		dir_close(searched_record.parent_dir);
		return -1;
...
int32_t sys_mkdir(const char* pathname) {
	...
	if(inode_no != -1) {
		...
		uint32_t path_searched_depth = path_depth_cnt(searched_record.searched_path);
		if(pathname_depth != path_searched_depth) {
			printk("sys_mkdir: cannot access %s: not a directory, subpath %s doesn't exist.\n", pathname, searched_record.searched_path);
			rollback_step = 1;
			goto rollback;
		}
	}
	...
	/* sync parent directory inode to hd */
	memset(io_buf, 0, SECTOR_SIZE * 2);
	inode_sync(cur_part, parent_dir->inode, io_buf);

	/* sync the new directory to hd */
	memset(io_buf, 0, SECTOR_SIZE * 2);
	inode_sync(cur_part, &new_dir_inode, io_buf);

	/* sync inode bitmap to hd */
...
struct dir* sys_opendir(const char* name) {
	...
	struct dir* ret = NULL;
	if(inode_no == -1) {
		printk("In %s, sub path %s doesn't exist.\n", name, searched_record.searched_path);
	} else {
