...
void make_clear_abs_path(char* path, char* final_path) {
	/* final_path = wash_path(cur_path + path)
	 * cur_path = getcwd()
	 */
...
/* [cwd]: get current path */
void builtin_pwd(uint32_t argc, char** argv UNUSED) {
	if(argc != 1) {
		printf("pwd: no argument supported!\n");
		return;
	} else {
...
char* builtin_cd(uint32_t argc, char** argv) {
	if(argc > 2) {
		printf("cd: only 1 argument supported!\n");
		return NULL;
	}
	if(argc == 1) {
		final_path[0] = '/';
	...
	while(arg_idx < argc) {
		if(argv[arg_idx][0] == '-') {
			...
			if(arg_path_nr == 0) {
				...
				printf("ls: only 1 path supported\n");
				return;
			}
		}
		++arg_idx;
		...
		if(long_info) {
			...
			while((dir_e = readdir(dir))) {
				...
				if(stat(sub_pathname, &file_stat) == -1) {
					printf("ls: cannot access %s: No fuch file or directory\n", dir_e->filename);
					return;
				}
...
void builtin_ps(uint32_t argc, char** argv UNUSED) {
	if(argc != 1) {
		printf("ps: no argument supported!\n");
		return;
	}
	ps();
}
...
void builtin_clear(uint32_t argc, char** argv UNUSED) {
	if(argc != 1) {
		printf("clear: no argument supported!\n");
		return;
	}
	clear();
}
...
int32_t builtin_mkdir(uint32_t argc, char** argv) {
	int32_t ret = -1;
	if(argc != 2) {
		printf("mkdir: only 1 argument supported!\n");
	} else {
		make_clear_abs_path(argv[1], final_path);
...
int32_t builtin_rmdir(uint32_t argc, char** argv) {
	int32_t ret = -1;
	if(argc != 2) {
		printf("rmdir: only 1 argument supported!\n");
	} else {
		make_clear_abs_path(argv[1], final_path);
		if(strcmp("/", final_path)) {
...
int32_t builtin_rm(uint32_t argc, char** argv) {
	int32_t ret = -1;
	if(argc != 2) {
		printf("rm: only 1 argument supported!\n");
	} else {
		make_clear_abs_path(argv[1], final_path);
		if(strcmp("/", final_path)) {
...
void builtin_test(uint32_t argc, char** argv UNUSED) {
	if(argc != 1) {
		printf("test: no argument supported!\n");
	}
	test();
}
